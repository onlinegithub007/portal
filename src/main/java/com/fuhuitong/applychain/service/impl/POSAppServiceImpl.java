...
//            resp.setPayBillNumber(order.getPayBillNumber());
    ...
    @Override
    public PosRepaymentResp posRepayment(POSPaySuccessReq reqParam, String ipAddress) {
        PosRepaymentResp resp = new PosRepaymentResp();
        ...
        if (StringUtils.isEmpty(orderCode)) {
            logger.warn("orderCode id is null!");
            resp.setRetCode("1");
            return resp;
        }
...
//        logger.info("merId is:[" + session.getMerId() + "]");
        ...
        Orders orders = this.ordersMapper.selectByOrderCode(query);
        if (orders == null) {
            logger.info("Cannot find orderCode: " + reqParam.getOrderCode());
            resp.setRetCode("1");
            return resp;
        }
            ...
            for (OrderDetails detail : orderDetails) {
                ...
                if (goodsStockDetails != null) {
                    for (GoodsStockDetail stockDetail : goodsStockDetails) {
                        ...
                        int stockCount = stockDetail.getStockCount().intValue();
                        stockCount += amount;

                        queryParam.setGoodsStockDetailId(stockDetail.getGoodsStockDetailId());
                        queryParam.setStockCount(stockCount);
                        //刷新详情里的库存
                        this.goodsStockDetailMapper.deCreaseStock(queryParam);
    ...
    public QueryOrderDetailResp posGetOrderDetail(QueryOrderDetailReq reqParam, String ipAddress) {
        ...
        resp.setPayMethod(order.getPayMethod());
        resp.setPayBillNumber(order.getPayBillNumber());
...
/*
    ...
    * *//*
    ...
    @Override
    public QueryOrderDetailResp posGetOrderDetailByBill(QueryOrderDetailReq reqParam, String ipAddress) {
        QueryOrderDetailResp resp = new QueryOrderDetailResp();
        ...
        // 判断查询日期是否完整
        String payBillNumber = reqParam.getPayBillNumber();
        if (StringUtils.isEmpty(payBillNumber)) {
            logger.warn("order code is null!");
            resp.setRetCode("1");
            return resp;
        }

        // 使用 MerId和payBillNumber查询
        Orders query = new Orders();
        query.setPayBillNumber(payBillNumber);
        query.setMerId(session.getMerId());

        logger.info("payBillNumber is:[" + payBillNumber + "]");
        logger.info("merId is:[" + session.getMerId() + "]");

        Orders order = this.ordersMapper.selectByPayBillNumber(query);
        ...
        resp.setRetCode("0");
        resp.setOrderCode(order.getOrderCode());
        resp.setOrderDatetime(DateFormatUtils.format(order.getPayDate(), "yyyy-MM-dd HH:mm:ss"));
...
//            s!=null&&s.length!=0?s:0
            ...
            if (order.getTotalPriceText()==null ||order.getTotalPriceText().trim()==null ) {
                resp.setActTotalAmount(0);
            }
            resp.setActTotalAmount(Integer.parseInt(order.getTotalPriceText()));
            if (order.getFinalPriceText()==null ||order.getFinalPriceText().trim()==null) {
                resp.setTotalAmount(0);
            }
            resp.setTotalAmount(Integer.parseInt(order.getFinalPriceText()));
            if (order.getDiscountAmountText()==null ||order.getDiscountAmountText().trim()==null) {
                resp.setDiscountAmount(0);
            }
            resp.setDiscountAmount(Integer.parseInt(order.getDiscountAmountText()));
        } catch (NumberFormatException e) {
            e.printStackTrace();
        }
        resp.setPayBillNumber(payBillNumber);
        resp.setPayMethod(order.getPayMethod());
